...
- 壁纸体验修复与改进：
  ...
  - 壁纸搜索结果卡片新增自定义右键菜单（预览大图 / 下载并应用 / 复制原图 URL / 拉黑），替代失效的系统原生图片菜单；与卡片「⋯」菜单共用同一套动作
  - 右键菜单在光标处弹出并视口内收敛，点击外部 / Esc / 滚动 / 切换卡片时关闭，且不会误触发预览
  - 修复搜索后缩略图反复整体闪刷：`thumb-ready` 事件携带 hash，前端仅重新加载对应缩略图，其余图片零影响
